...
module.exports = async (req, res) => {
  ...
  const cidIdsQuery = await db.query(
    `UPDATE maintenancedb SET notes = '${value}', updatedBy = '${updatedBy}' WHERE id = ${maintId}`
  )
  if (cidIdsQuery.affectedRows >= 1) {
    const updateHistory = await db.query(
      escape`INSERT INTO changelog (mid, user, action, field) VALUES (${maintId}, ${updatedBy}, 'changed', 'notes');`
    )
